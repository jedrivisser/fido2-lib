...
async function validateOrigin() {
    ...
    if (Array.isArray(expectedOrigin)) {
        if (!expectedOrigin.indexOf(origin) < 0) {
            throw new Error("clientData origin did not match expected origins");
        }
...
async function validateRpIdHash() {
    ...
    var rpId = this.expectations.get("rpId")
    var createdHash = new Uint8Array(crypto.createHash("sha256").update(rpId).digest()).buffer;
